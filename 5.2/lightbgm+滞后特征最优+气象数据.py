...
if __name__ == '__main__':
    ...
    all_features = feature_cols + ['hour', 'dayofweek', 'month', 'price_lag_96'] + weather_feature_cols
    # 填充气象特征的缺失值
    ...
    # ==================== 2. 模型训练（网格搜索，微调参数） ====================
    print("\n[2/4] 训练 LightGBM 模型（网格搜索：alpha/lambda/depth，基于RMSE）...")
    import lightgbm as lgb
    ...
    # 更细粒度的搜索范围（仅关闭早停，确保与之前的最优配置一致）
    param_grid = {
        'reg_alpha': [0.035],
        'reg_lambda': [0.045],
        'max_depth': [4],
        'early_stopping_rounds': [None]   # None = 关闭早停
    }
            ...
            for max_depth in param_grid['max_depth']:
                for es_rounds in param_grid['early_stopping_rounds']:
                    ...
                    if es_rounds is None:
                        n_estimators = 200          # 关闭早停，固定树数
                        use_early_stop = False
                        es_desc = "早停关"
                    else:
                        n_estimators = 1000
                        use_early_stop = True
                        es_desc = f"早停({es_rounds}轮)"

                    print(f"  测试 alpha={reg_alpha:.3f}, lambda={reg_lambda:.3f}, "
                          f"depth={max_depth}, {es_desc} ...", end=" ")
    ...
    # 测试集没有真实滞后特征，填充 0（模型会忽略这个特征，因为全是同一个值）
