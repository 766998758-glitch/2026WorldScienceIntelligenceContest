"""
LightGBM 两步走基线 + 气象特征 + 预测偏差特征 + 网格搜索最优参数
"""
...
# 实际值特征列（用于构造偏差特征）
feature_cols_actual = ['系统负荷实际值', '风光总加实际值', '联络线实际值',
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("LightGBM + 气象特征 + 预测偏差特征 + 网格搜索")
    print("=" * 60)
    ...
    print("构造预测偏差特征...")
    bias_cols = []
    for act, pred in zip(feature_cols_actual, feature_cols):
        if act in df_train.columns and pred in df_train.columns:
            bias_name = f'{pred}_偏差'
            df_train[bias_name] = df_train[act] - df_train[pred]
            bias_cols.append(bias_name)

    # ----- 构造训练集“昨天同一时刻电价”作为训练辅助特征 -----
    df_train['price_lag_96'] = df_train[target_col].shift(96)
    df_train = df_train.dropna(subset=['price_lag_96']).reset_index(drop=True)
    print(f"  加入滞后特征后训练集: {df_train.shape}")
    ...
    # 最终特征列：预测值 + 时间特征 + 气象特征 + 偏差特征
    all_features = feature_cols + ['hour', 'dayofweek', 'month'] + weather_feature_cols + bias_cols + ['price_lag_96']
    # 填充气象缺失值
    ...
    print(f"  特征列总数: {len(all_features)} (包含 {len(bias_cols)} 个偏差特征)")
    ...
    # 测试集没有实际值，偏差特征全部填0
    for bias_name in bias_cols:
        df_test[bias_name] = 0.0
        df_test['price_lag_96'] = 0.0
